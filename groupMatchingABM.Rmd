...
results <- data.frame("generation" = rep(0:generations, each = length(all_combos)),
...
for(m in 1:modelLoops) {
  ...
  while(g <= generations){
    ...
    results$population[results$generation == g] <- stratTable$Freq[match(results$combo, stratTable$Var1)]
      
    print(stratTable)
    
    if (max(results$population) >= 160) {
