...
overlap_prims <- function(adjMat){
  ...
  return(list(A = allianceMat, G = generalizationMat, P = displacementMat, D = defenseMat))
...
overlap_mean <- function(agent, agentDF, matrixList){
 ...
 #for a given agent:
  ...
  selectedMats <- matrixList[selectedPrims]
    ...
    means <- ifelse(is.null(row_values), 1, colMeans(row_values))
...
mutateBabies <- function(agentDF, flipProb) {
  ...
  for (a in 1:nrow(agentDF)) {
    ...
    agentDF$strat[a] <- paste(names(agentDF)[agentDF[a,] == 1],collapse="")
  ...
  while(g <= generations){
    ...
    for (t in 2:reassessments) {
      ...
      for (a in 1:nrow(agents)) {
        ...
        association_values <- agents$association
...
write.csv(results_may18, "C:/Users/clavra/Documents/groupFormation/results_may18.csv")
...
# graph_list <- lapply(tiesMatrix_list, function(mat) {
#   graph_from_adjacency_matrix(mat, mode = "directed", diag = FALSE)
# })
#add vertex attributes

tiesMat_list <- lapply(tiesMat_list, function(mat) {
  rownames(mat) <- colnames(mat) <- as.character(1:160)
  mat
})

igraph_list <- lapply(tiesMat_list, graph_from_adjacency_matrix, add.rownames = T)
...
for(r in 1:length(igraph_list)) {
  ...
  #V(igraph_list[[r]])$name <- c(1:160)
  
  V(igraph_list[[r]])$association <- agents$association[match(V(igraph_list[[r]])$name, rownames(agents))]
  
   V(graph_list[[r]])$strat <- agents$strat[match(V(igraph_list[[r]])$name, rownames(agents))]
  
  V(graph_list[[r]])$color <- agents$stratColor[match(V(igraph_list[[r]])$name, rownames(agents))]
