...
overlap <- function(agent, adjMat) {
  ...
  alliance <- apply(adjMat, 1, function(x) {
    ...
    #exclude agent and x's columns from their row vectors
    x_filtered <- x[-exclude]
    agent_row_filtered <- adjMat[agent, -exclude]
    
    # calculate Weighted Jaccard Index on filtered data
  ...
  displacement <- apply(adjMat, 2, function(x) {
    ...
    #exclude agent and x's rows from x column
    x_filtered <- x[-exclude]
    
    #exclude agent and x's columns from agent's row
    agent_filtered <- adjMat[agent, -exclude]
  ...
  defense <- apply(adjMat, 1, function(x) {
    ...
    #exclude agent and x's columns from x row
    x_filtered <- x[-exclude]
    
    #exclude agent and x's rows from agent's column
    agent_filtered <- adjMat[-exclude, agent]
  ...
  generalization <- apply(adjMat, 2, function(x) {
    ...
    #exclude agent and x's rows from their columns
    x_filtered <- x[-exclude]
    agent_filtered <- adjMat[-exclude, agent]
...
igraph::components(igraph_list[[155]])
...
clique_results <- data.frame("reassessment" = 1:reassessments,
                             "percentInClique" = NA)
#for each reassessment period...
for (r in 1:length(igraph_list)) {
  
  #...get percent of agents in a clique
  percentInClique <- length(unlist(max_cliques(igraph_list[[r]], min = 3)))/100
...
ggplot(data = clique_results,
  ...
  labs(title = "Percent of agents in cliques per reassessment",
       subtitle = "(reciprocity(0-1) + overlap(0-4)) * alliance(0-1)",
       x = "reassessment period",
...
render.d3movie(dynamicNet, filename = "dynNet.html")
