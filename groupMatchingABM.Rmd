...
#roles
roles <- rep(c("fire","air","earth","water"), 25)
...
agentGenerate <- function(n){
  ...
  alliance <- round(alliance, digits = 0)/100
  
  role <- sample(roles, n, replace = F)
    
  pursuit_amount <- sample(2:5, n, replace = T)
  
  #put it all together in a df
  agents <- data.frame(alliance, role)
...
results <- data.frame(
    "loop" = rep(1:modelLoops, each = reassessments),
...
for(m in 1:modelLoops) {
  ...
  for (t in 2:reassessments) {
    ...
    for (a in 1:nrow(agents)) {
      ...
      #combine pursuit scores with the agents' roles
      scores_roles <- data.frame("score" = pursuit_scores, "role" = agents$role, "index" = c(1:100))
      
      #pull agent a's role
      agentRole <- agents$role[a]
      
      #get indices of highest pursuit values within each role
      max_scores <- scores_roles %>%
        ...
        #get row with max alliance in each group
        slice(which.max(score)) %>%  
        ungroup() %>%
        
        select(role, index, score) 
      
      #get vector of pursuit indices from previous df, excluding index of agent with matching role
      pursuit_indices <- max_scores$index[!max_scores$role == agentRole]
  ...
  for (g in 1:length(igraph_list)) {
    ...
    results$numEdges[results$reassess == g & results$loop == m] <- numEdges
    results$intersectLast[results$reassess == g & results$loop == m] <- intersectLast
    results$num_groups[results$reassess == g & results$loop == m] <- num_groups
    results$num_isolates[results$reassess == g & results$loop == m] <- num_isolates
    results$num_four[results$reassess == g & results$loop == m] <- num_four
    results$assort_alliance[results$reassess == g & results$loop == m] <- assortativity(igraph_list[[g]], V(igraph_list[[g]])$alliance)
...
ggplot(data = results,
  ...
  labs(title = "",
       ...
       y = "percent of edges that persisted from previous reassessment period",
       color = "loop") +
  geom_smooth(se= F)
...
##temporary##
agents$color <- NA
agents$color[agents$role == "earth"] <- "green"
agents$color[agents$role == "water"] <- "blue"
agents$color[agents$role == "fire"] <- "red"
agents$color[agents$role == "air"] <- "yellow"
...
for(r in 1:length(igraph_list)) {
  ...
  V(igraph_list[[r]])$role <- agents$role[match(V(igraph_list[[r]])$name, rownames(agents))]
...
assortativity(igraph_list[[200]], V(igraph_list[[200]])$alliance)
