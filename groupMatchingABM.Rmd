...
reproduce <- function(agentDF, selStrength, n){
  ...
  for (a in 1:nrow(offspringDF)) {
    ...
    offspringDF$A <- rnorm(1, mean = offspringDF$A[a], sd = .05)
    offspringDF$G <- rnorm(1, mean = offspringDF$G[a], sd = .05)
    offspringDF$P <- rnorm(1, mean = offspringDF$P[a], sd = .05)
    offspringDF$D <- rnorm(1, mean = offspringDF$D[a], sd = .05)
...
for(m in 1:modelLoops) {
  ...
  while(g <= generations){
    ...
    print(c("A",avWeight_A, "G",avWeight_G, "P",avWeight_P, "D",avWeight_D))
