...
agentGenerate <- function(n){
  ...
  pursuit_amount <- sample(2:5, n, replace = T) #change this
  
  #assign agents an overlap strategy
  strat <- sample(primies$prim_abbrev, n, prob = primies$prop, replace = T)
...
overlap_mean <- function(agent, agentsDF, matrixList){
  ...
  row_values <- do.call(rbind, lapply(subsetMats, function(mat) mat[agent, ]))
...
#Contributions#
#sum of the group's contributions divided equally among members

payoff <- function(gameMatrix, agentDF){
  ...
  for(agent in groupIDs){
    ...
    proportionTied <- sum(gameMatrix[as.character(agent), ])/nrow(gameMatrix)
  ...
  payout <- contributionTot/nrow(matrix)
...
for(m in 1:modelLoops) {
  ...
  agents <- agentGenerate(popSize)
    ...
    tiesMatrix_list <- list(pursuitMatrix)
    ...
    for (t in 2:reassessments) {
      ...
      for (a in 1:nrow(agents)) {
        ...
        pursuit_scores <-
          overlap_scores * reciprocity_scores * alliance_values
    ...
    for (g in 1:length(igraph_list)) {
      ...
      # results$assort_alliance[results$reassess == g & results$combo == c & results$loop == m] <- assortativity(igraph_list[[g]], V(igraph_list[[g]])$alliance)
      # 
