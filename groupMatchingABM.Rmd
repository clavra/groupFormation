...
payout <- function(matrix, agentDF){
  
  contributionTot <- 0
  
  groupIDs <- rownames(matrix)
  
  for(agent in groupIDs){
    
    maxContribution <- agentDF$alliance[row.names(agentDF) == agent] * 100
    
    proportionTied <- sum(matrix[as.character(agent), ])/nrow(matrix)
...
for(m in 1:modelLoops) {
  ...
  agents <- agentGenerate(popSize)
    ...
    for (t in 2:reassessments) {
      ...
      overlap_matrices <- overlap_prims(matrix_list[[t-1]])
      ...
      gamersMat <- random_matrix[gamerGroup, gamerGroup, drop = F]
