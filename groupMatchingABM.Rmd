...
payoff <- function(gameMatrix, agentDF){
  ...
  pay <- contributionTot/nrow(gameMatrix)
...
results <- data.frame("generation" = rep(1:generations, each = length(all_combos)),
                      "combo" = rep(prim_abbrev),
                      "amount" = NA)

resultsbind <- data.frame("generation" = rep(11:50, each = length(all_combos)),
                      "combo" = rep(prim_abbrev),
                      "amount" = NA)

results <- rbind(results, resultsbind)

results$amount[results$generation == 1] <- 10
...
for(m in 1:modelLoops) {
  ...
  for(g in 11:20){
    ...
    for(gamer in 1:popSize){
      ...
      payoffs[gamerGroup] <- payoffs[gamerGroup] + pay
    ...
    results$amount[results$generation == g+1] <- stratTable$Freq[match(results$combo, stratTable$Var1)]
