...
results <- data.frame("loop" = rep(1:modelLoops, each = 2*(generations)),
                      "generation" = rep(1:generations, each = 2),
...
for(m in 1:modelLoops) {
  ...
  while(g <= generations){
    ...
    # igraph <- graph_from_adjacency_matrix(friendsMatrix)
    # 
    ...
    #add the populations to the results df
    results$population[results$generation == g & results$loop == m] <- stratTable$Freq[match(prim_abbrev, stratTable$Var1)]
      
    #add the average association value of each strat to the results df
    results$avgAssoc[results$generation == g & results$loop == m & results$combo == "OV"] <- mean(agents$association[agents$strat == "OV"])
    results$avgAssoc[results$generation == g & results$loop == m & results$combo == "XX"] <- mean(agents$association[agents$strat == "XX"])

    #add the average payoff of each strat to the results df
    results$avgPay[results$generation == g & results$loop == m & results$combo == "OV"] <- mean(agents$payoff[agents$strat == "OV"])
    results$avgPay[results$generation == g & results$loop == m & results$combo == "XX"] <- mean(agents$payoff[agents$strat == "XX"])
...
lastGraph <- lastGraphs[[1]]

for(i in 1:length(V(lastGraph))){
  
  V(lastGraph)$color[i] <- ifelse(V(lastGraph)$strat[i] == "OV", "blue","red")
