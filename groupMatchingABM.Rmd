...
myPath <- "G:/My Drive/groupFormationResults/noisyLong.csv"
...
for(m in 1:modelLoops) {
  ...
  while(g <= generations){
    ...
    #get network data
    networkGraph <- graph_from_adjacency_matrix(friendsMatrix, mode = "undirected")
    results$cliques[results$generation == g & results$loop == m] <- count_max_cliques(networkGraph, min = 4, max = 4)
...
resultsLong <- pivot_longer(data = results,
...
p3 <- ggplot(data = resultsLong,
             aes(x = generation,
                 y = mean,
                 color = prim)) +
  labs(x = "generation",
       y = "average % of agents using prim") +
  geom_smooth(alpha = 0.2) +
  theme_minimal()


p4 <- ggplot(data = resultsLong,
             aes(x = generation,
                 y = mean,
                 color = prim)) +
  labs(x = "generation",
