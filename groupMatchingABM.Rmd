...
overlap_prims<- function(adjMat) {
  ...
  for (agent in 1:popSize) {
    ...
    if (agent < popSize) {
      ...
      #only filling out top triangle (will fold later)
    ...
    #filling out the whole thing (defense mat = transposition of displacement mat)
    for (j in 1:popSize) {
      
      #skip the agent's row
      if (j == agent) next
      exclude_indices <- c(agent, j)
      agent_filtered <- agent_row[-exclude_indices]
      other_filtered <- adjMat[-exclude_indices, j]
...
append_shift <- function(lst, new_elem) {
    lst <- c(lst[-1], list(new_elem))  # drop first, append new
    return(lst)
}
...
#get the flat indices of the agent's rows and cols in matrix
...
updateTiesMats <- function(agent, indiv)
...
makeNoisyNewTies <- function(agent, godTiesMatrix, obscurePercent) {
    ...
    #remove the 1 indicies that are the agent's direct pursuits
    indPositivePursuits <- positivePursuits[!positivePursuits %in% agentIxs]
    
    #get the indicies of the 75% of indirect 1s that we are gonna turn into 0s
...
#Updating Individual Pursuit Mats#
updatePursuitMat <- function(agent, indivPursuitMats, indivTiesMatsList, newTies){
  
    #exclude the data from 5 rounds ago held in their indiv ties mat
    indivPursuitMats[[agent]] <- indivPursuitMats[[agent]] - indivTiesMatsList[[agent]][[1]]
    
    #add the new noisy ties 
    indivPursuitMats[[agent]] <- indivPursuitMats[[agent]] + noisyNewTies
    
    return(indivPursuitMats)
...
for(m in 1:modelLoops) {
  ...
  while(g <= generations){
    ...
    indivTiesMats <- replicate(popSize, simplify = F, 
                                    replicate(5, matrix(0, nrow = popSize, ncol = popSize), 
                                              simplify = FALSE))
  
    #initiate the individual pursuit mats
    indivPursuits <- replicate(popSize, matrix(0, nrow = popSize, ncol = popSize))
    
    
    #for each reassessment... (agents look backwards so start from 2)
    for (t in 2:reassessments) {
      
      #create matrix that holds every definitive pursuit per round
      ...
      #Updating pursuit mat to be used in next round
      
      #append noisy ties from godTiesMat to each agent's ties matrices list, deleting the 5th last one
      
      for(a in 1:popSize){
        
        #create new noisy ties from past round
        newTies <- makeNoisyNewTies(a, godTiesMatrix, obscurePercent)
        
        #update the pursuit mat they will use for the next round
        updatePursuitMat(a, indivPursuits[[a]], )
      ...
      #update each agent's indiv pursuit matrix (which they'll use to calculate overlap in next round)
      
      
      
      
      #add their current pursuits to their individual ties matrix list, shifting all left
      append_shift(indiv_ties_mats[[a]], )
      
      
      #for each agent, turn 75% of the god ties matrix into 0s and then add those 1s
