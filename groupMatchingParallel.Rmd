...
reproduce <- function(agentDF, selStrength, n){
 ...
 #Mutations:
  ...
  association_values <- rnorm(n, mean = offspringDF$association, sd = .05)
...
results <- foreach(m = 1:length(agentDFs), .combine = rbind) %dopar% {
  ...
  resultsDF$population[resultsDF$generation == 0 & resultsDF$strat == "mean"] <- stratTableOG["1"]
  resultsDF$population[resultsDF$generation == 0 & resultsDF$strat == "min"] <- stratTableOG["2"]
