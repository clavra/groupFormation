...
=======
#selStrength <- 3
>>>>>>> 3f80c3e8e251a095a2b6435d9a33f413e93213c3
...
#the group size required in order to play the game
#sizeReq <- 4

#path to save results file
<<<<<<< HEAD
myPath <- "G:/My Drive/groupFormationResults/parameterChange.csv"
=======
myPath <- "G:/My Drive/groupFormationResults/params.csv"
>>>>>>> 3f80c3e8e251a095a2b6435d9a33f413e93213c3

                    

<<<<<<< HEAD
parameterDF <- data.frame("pursuitAmount" = c(3,3,5,5,7,7),
                          "sizeReq" = c(4,3,6,4,8,4)
=======
parameterDF <- data.frame("pursuitAmount" = c(3,3,3,3,7,7,7,7),
                          "sizeReq" = c(4,3,4,3,8,4,8,4),
                          "selStrength" = c(1,1,4,4,1,1,4,4)
>>>>>>> 3f80c3e8e251a095a2b6435d9a33f413e93213c3
                         )
...
# resultsDF <- data.frame("loop" = rep(1:modelLoops, each = (generations + 1)*nrow(parameterDF)),
#                       "parameterInd" = rep(1:nrow(parameterDF), each = generations + 1),
...
results <- foreach(m = 1:length(agentDFs), .combine = rbind) %dopar% {
  ...
  #each set of starting agents will be run using different parameters
  
  for (p in 1:nrow(parameterDF)) {
    
    #Generate parent agents
    agents <- agentDFs[[m]]
  
    #Parameters
    sizeReq <- parameterDF[p, "sizeReq"]
    pursuitAmount <- parameterDF[p, "pursuitAmount"]
    selStrength <- parameterDF[p, "selStrength"]
    ...
    while (g <= generations) {
      ...
      for (t in 2:reassessments) {
        ...
        for (i in sample(1:popSize)) {
          ...
          if (nrow(gamersMat) == sizeReq & prod(rbinom(nrow(gamersMat), 1, rowSums(gamersMat) / nrow(gamersMat))) == 1) {
      ...
      popOV <- stratTable$Freq[stratTable$Var1 == "OV"]
      
      resultsDF$population[resultsDF$generation == g & resultsDF$parameterInd == p] <- popOV
...
#add the parameter details to the results

resultsAdd <- results

parameterDF$parameterInd <- 1:nrow(parameterDF)

resultsAdd$mutationRate <- 0.01
resultsAdd$selStrength <- 4

resultsAdd$pursuitAmount <- parameterDF$pursuitAmount[match(resultsAdd$parameterInd, parameterDF$parameterInd)]
resultsAdd$sizeReq <- parameterDF$sizeReq[match(resultsAdd$parameterInd, parameterDF$parameterInd)]

write.csv(resultsAdd, myPath)
