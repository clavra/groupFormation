...
reproduce <- function(agentDF, selStrength, n){
 ...
 #Mutations:
  ...
  association_values <- rnorm(1, mean = offspringDF$association, sd = .05)
...
results <- foreach(m = 1:length(agentDFs), .combine = rbind) %dopar% {
  ...
  resultsDF$population[resultsDF$generation == g & resultsDF$strat == "mean"] <- stratTableOG["1"]
  resultsDF$population[resultsDF$generation == g & resultsDF$strat == "min"] <- stratTableOG["2"]
  ...
  while(g <= generations){
    ...
    #add these to resultsDF DF
    resultsDF$population[resultsDF$generation == g & resultsDF$strat == "mean"] <- stratTable["1"]
    resultsDF$population[resultsDF$generation == g & resultsDF$strat == "min"] <- stratTable["2"]
